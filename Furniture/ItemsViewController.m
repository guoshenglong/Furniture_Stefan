...
- (id)init
{
    ...
    if (self) {
        ...
        **/
        // 自定义左侧按钮
        // 创建左侧按钮的背景图片
        UIImage *lBackImage = [UIImage imageNamed:@"Lbutton.png"];
        UIImage *lBackImage_black = [UIImage imageNamed:@"Lbutton_black.png"];
        CGRect lBackframe = CGRectMake(0, 0, 31, 31);
        UIButton *lBackButton = [UIButton buttonWithType:UIButtonTypeCustom];
        lBackButton.frame = lBackframe;
        // 为左侧按钮指定背景图片和大小
        [lBackButton setImage:lBackImage forState:UIControlStateNormal];
        // 指定高亮图片
        [lBackButton setImage:lBackImage_black forState:UIControlStateHighlighted];
        // [lBackButton setTitle:@"返回" forState:UIControlStateNormal];
        lBackButton.titleLabel.font = [UIFont systemFontOfSize:13];
        [lBackButton addTarget:self action:@selector(leftButtonAction:) forControlEvents:UIControlEventTouchUpInside];
        UIBarButtonItem *leftBarButton = [[UIBarButtonItem alloc] initWithCustomView:lBackButton];
...
{
    DetialViewController *detailViewController = [[DetialViewController alloc] init];
